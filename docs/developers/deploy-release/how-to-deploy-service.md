...
SmartTouch uses a GitOps deployment model. You push code; GitHub Actions builds and tests it; Argo CD detects the manifest change and deploys to the target environment. You do not run `kubectl apply` or access the cluster directly.
...
The manifest update is committed automatically by the pipeline to the `k8s/overlays/dev/` overlay. You do not need to update the overlay manually.
