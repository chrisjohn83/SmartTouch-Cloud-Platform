...
This method reverts the cluster state immediately without a pull request. Use it when an incident is active and every minute matters.
...
## Method 2 — Auditable rollback via git revert
...
The service has only one deployment revision. There is nothing to roll back to. Deploy a known-good image tag directly:
