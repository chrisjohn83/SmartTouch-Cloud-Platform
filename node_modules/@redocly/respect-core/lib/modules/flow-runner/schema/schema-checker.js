...
export function checkSchema({ stepCallCtx, descriptionOperation, ctx, ajvContext, }) {
...
function checkSchemaFromDescription({ checks, descriptionOperation, $response, ctx, ajvContext, }) {
    ...
    if (schemaFromDescription && !isSchemaWithCircularRef) {
        try {
            const validate = ajvStrict.compile(schemaFromDescription);
            checks.push({
                name: CHECKS.SCHEMA_CHECK,
                passed: validate.call(ajvContext, resultBody),
                message: validate.errors
                    ? printAjvErrors(schemaFromDescription, resultBody, validate.errors)
                    : '',
...
function contentTypeMatchesPattern(contentType, pattern) {
    const normalizedPattern = pattern.toLowerCase();
    const normalizedContentType = contentType.toLowerCase();
    if (normalizedPattern === '*/*')
        return true;
    if (normalizedPattern === normalizedContentType)
        return true;
...
export function resolveContentByType(content, contentType) {
    if (!content)
        return undefined;
...
function checkContentTypeFromDescription({ checks, descriptionOperation, $response, ctx, }) {
    ...
    if (responseContentType &&
        !possibleContentTypesFromDescription.some((pattern) => contentTypeMatchesPattern(responseContentType, pattern))) {
        checks.push({
