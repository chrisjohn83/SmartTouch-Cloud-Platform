...
*/
var objectToFormParams = (obj, encoding, parentKey, isMultipart = false) => {
	const params = [];
	/** Ensure we do not include disabled items */
	const entries = Array.isArray(obj) ? obj.filter((item) => !item.isDisabled).map((item) => [String(item.name), item.value]) : Object.entries(obj);
	for (const [key, value] of entries) {
		if (value === void 0 || value === null) continue;
		const partEncoding = parentKey ? void 0 : encoding?.[key];
		/**
		...
		*/
		const hasFormStyle = partEncoding && (partEncoding.style !== void 0 || partEncoding.explode !== void 0 || partEncoding.allowReserved !== void 0);
		/**
...
var processBody = ({ requestBody, contentType, example, requestBodyCompositionSelection }) => {
	...
	if (typeof _example !== "undefined") {
		...
		if (isFormData && typeof exampleValue === "object" && exampleValue !== null) return {
			...
			params: objectToFormParams(exampleValue, encoding, void 0, _contentType === "multipart/form-data")
		};
	...
	if (typeof contentSchema !== "undefined") {
		...
		if (extractedExample !== void 0) {
			if (isFormData && typeof extractedExample === "object" && extractedExample !== null) return {
				mimeType: harMimeType,
				params: objectToFormParams(extractedExample, encoding, void 0, _contentType === "multipart/form-data")
			};
